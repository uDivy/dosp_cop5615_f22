    ...
    receive
        ...
        {Gossip, Sender_id, Neighbour, Pos, LineTopology, S, W, Count, connectline} ->
            ...
            if Ratio =< Thresh ->
                ...
                true ->
                    if Count == 3 ->
                        lists:nth(2,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))) ! {"youknowwhat", Sender_id, Neighbour, lists:nth(1,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))), LineTopology, Snew/2, Wnew/2, 0, connectline},
                        exit(self());
                    true ->
                        io:format("~w ~n",[Count]),
                        lists:nth(2,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))) ! {"youknowwhat", Sender_id, Neighbour, lists:nth(1,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))), LineTopology, Snew/2, Wnew/2, Count + 1, connectline}
                    end
                end;
            true ->
                % io:format("My name is ~w and I heard ~p from ~w ~w times~n",[self(), Gossip, Sender_id, Count]),
                ...
                case is_process_alive(Heardby) of 
                    false -> 
                        ...
                        if Val == length(LineTopology) ->
                            ...
                            exit(self());
                        true ->
                            lists:nth(2,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))) ! {"youknowwhat", Sender_id, Neighbour, lists:nth(1,random_selection_of_living_actors(self(), 1, Sender_id, LineTopology, length(LineTopology))), LineTopology, Snew/2, Wnew/2, Count, connectline}
                        end;
                    _ ->
                        Heardby ! {"youknowwhat", Sender_id, Neighbour, Posnew, LineTopology, (Snew)/2, (Wnew)/2, Count, connectline}
                end
            end
    end,
    listen_ps(Si, Wi).
...
build_line_pushsum(NumNodes, Rest, Row) ->
    Pid = spawn(pushsum, listen_ps, [NumNodes, 1]),
    Pid ! {"Nothing", self()},
    receive
    ack -> 
        % io:format("The new member in topology is: ~w~n",[Pid])
    ...
    build_line_pushsum(NumNodes-1, [Pid | Rest], Row+1).
